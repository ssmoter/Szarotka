...
{
    public class DayRangeCalculationAverageTest
    {
        private readonly IList<DayExpanded> dayExpanded;

        public DayRangeCalculationAverageTest()
        {
            dayExpanded = [];
        ...
        {
            var result = Inventory.Helper.Calculations.DayRangeCalculationAverage.Week(dayExpanded);
            ...
            Assert.Equal(2, second!.Day.TotalPriceAfterCorrectDecimal);
        }
        [Fact]
        public void Month_Should_Calculate()
        {
            var result = Inventory.Helper.Calculations.DayRangeCalculationAverage.Month(dayExpanded);
        ...
        {
            var result = Inventory.Helper.Calculations.DayRangeCalculationAverage.Year(dayExpanded);
        ...
        {
            var result = Inventory.Helper.Calculations.DayRangeCalculationAverage.DayOfWeek(dayExpanded);
        ...
        {
            var result = Inventory.Helper.Calculations.DayRangeCalculationAverage.All(dayExpanded);

            Assert.Equal(2, result.Count());
            var first = result.FirstOrDefault();
            var second = result.LastOrDefault();
            Assert.Equal(2, first!.Day.TotalPriceDecimal);
            Assert.Equal(2, second!.Day.TotalPriceDecimal);
        }
    }

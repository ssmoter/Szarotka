...
namespace DataBase.Data
{
    public class InventoryTables
    {
        ...
        readonly Random _random = new(2137);

        public InventoryTables(AccessDataBase db)
        {
            _db = db;
        }
        Guid GetGuidSed()
        ...
        private async Task CreateDefaultProduct()
        {
            ...
            {
                var task = TaskProduct(products[i], i);
                taskTableName[i] = task.name;
                taskTablePrice[i] = task.price;
            }
        ...
        private (Task name, Task price) TaskProduct(Product product, int Arrangement)
        {
            ...
            task.name = _db.DataBaseAsync.InsertOrReplaceAsync(product.Name);
            task.price = _db.DataBaseAsync.InsertOrReplaceAsync(product.Price);
            return task;
        }
